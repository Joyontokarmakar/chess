...
const OnlineGameSetup: React.FC<OnlineGameSetupProps> = ({ onGameSetupComplete, onBackToMenu, theme, initialTimeLimit }) => {
  ...
  // Styling (can be kept as is or adjusted)
  ...
  const handleCreateGame = () => {
    ...
    const gameStartTs = initialTimeLimit ? Date.now() : null;
    const initialHostState: OnlineGameState = {
      ...
      timeLimitPerPlayer: initialTimeLimit,
      player1TimeLeft: initialTimeLimit,
      player2TimeLeft: initialTimeLimit,
      gameStartTimeStamp: gameStartTs,
    };
    try {
      setOnlineGameState(newGameId, initialHostState);
      setCreatedGameId(newGameId);
      setIsLoading(false); 
      setCopied(false);
    } catch (e) {
      setError("Error creating game. LocalStorage might be full or unavailable.");
      console.error(e);
      setIsLoading(false);
    }
  };
  ...
  const proceedToHostGame = () => {
    ...
    if (hostState) {
      onGameSetupComplete(createdGameId, true, hostState.player1Name, hostState);
    } else {
      setError("Error starting game. Host state not found. Please try creating again.");
      setIsLoading(false);
    }
  }
  ...
  const handleJoinGame = () => {
    ...
    try {
      ...
      if (existingState) {
        ...
        const updatedStateForJoin: OnlineGameState = { 
          ...existingState,
        ...
        // Ensure player 2 time is set if host had a time limit
        if (existingState.timeLimitPerPlayer && !existingState.player2TimeLeft) {
            updatedStateForJoin.player2TimeLeft = existingState.timeLimitPerPlayer;
        }
        
        setOnlineGameState(gameToJoinId, updatedStateForJoin);
        setIsLoading(false);
        onGameSetupComplete(gameToJoinId, false, playerName.trim(), updatedStateForJoin);
      } else { 
        setError('Game ID not found. Please check the ID and try again.'); 
        setIsLoading(false); 
      }
    } catch (e) {
      setError("Error joining game. LocalStorage might be full or unavailable.");
      console.error(e);
      setIsLoading(false);
    }
  };
    ...
    return (
       <div className="min-h-screen flex flex-col items-center justify-center bg-transparent p-4">
        <div className={`backdrop-blur-xl shadow-2xl p-7 sm:p-10 rounded-xl max-w-md w-full text-center ${panelBgClass}`}>
            ...
            <p className={`${textColorClass} mb-3 text-sm`}>Share this Game ID with your friend (playing on the same device/browser):</p>
            <div className="flex items-center justify-center space-x-3 my-5">
             ...
             {initialTimeLimit && <p className={`text-xs sm:text-sm mb-2 ${theme === 'dark' ? 'text-slate-400' : 'text-slate-600'}`}>Time: {initialTimeLimit / 60} minutes per player.</p>}
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-transparent p-4">
      <div className={`backdrop-blur-xl shadow-2xl p-7 sm:p-10 rounded-xl max-w-md w-full ${panelBgClass}`}>
        <h2 className={`text-2xl sm:text-3xl font-bold mb-8 text-center ${titleColorClass}`} style={{ textShadow: titleShadowClass}}>Play Online (Same Device)</h2>
        {!mode && (
        ...
        {mode && (
          <form onSubmit={(e) => e.preventDefault()} className="space-y-6">
            <h3 className={`text-xl font-semibold text-center ${theme === 'dark' ? 'text-slate-200' : 'text-slate-700'}`}>
             ...
             {mode === 'create' && initialTimeLimit && (
              <p className={`text-sm text-center ${textColorClass}`}>
                Selected time: {initialTimeLimit / 60} minutes per player.
              </p>
            )}
            <div>
