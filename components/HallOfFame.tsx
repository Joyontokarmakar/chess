...
import { HallOfFameEntry, Theme } from '../types'; 
...
const HallOfFame: React.FC<HallOfFameProps> = ({ onBackToMenu, theme }) => {
  ...
  const primaryButtonBaseClasses = `w-full sm:w-auto font-semibold py-3 px-7 rounded-lg text-base shadow-xl hover:shadow-2xl transition-all duration-200 ease-in-out transform hover:-translate-y-1 focus:outline-none focus-visible:ring-2 focus-visible:ring-opacity-75 text-white`; // Added text-white here for all
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-transparent p-4">
      <div className={`backdrop-blur-xl shadow-2xl p-6 sm:p-8 rounded-xl w-full max-w-3xl ${panelBgClass}`}>
          ...
          <div className={`overflow-x-auto rounded-lg border shadow-inner ${tableContainerBorderClass}`}>
            <table className={`min-w-full divide-y ${tableDivideClass}`}>
              <thead className={tableHeaderBgClass}>
                <tr>
                  {['Rank', 'Winner', 'Opponent', 'Mode', 'Date'].map(header => (
              ...
              <tbody className={`divide-y ${tableDivideClass}`}>
                {entries.map((entry, index) => (
                  ...
                  >
                    ...
                    <td className={`px-4 py-3 whitespace-nowrap text-sm ${getRowOtherTextColor} capitalize`}>{entry.mode === 'computer' ? 'vs AI' : (entry.mode === 'loaded_friend' ? 'Loaded Game' : entry.mode)}</td>
                    <td className={`px-4 py-3 whitespace-nowrap text-sm ${getRowOtherTextColor}`}>{entry.date}</td>
                  </tr>
                ))}
              </tbody>
            </table>
